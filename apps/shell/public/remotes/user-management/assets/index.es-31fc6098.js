...
import { ref, computed, defineComponent, shallowRef, onMounted, openBlock, createBlock, resolveDynamicComponent, createElementBlock, defineAsyncComponent, reactive, onUnmounted, Fragment, renderList, unref, withCtx, createElementVNode, toDisplayString } from 'vue';
...
const translationDataSource = ref(null);
const currentLangId = ref(null);
...
const decimalSeperators = ref([]);
computed(() => userSettings.value?.language?.id ?? appSettings.value?.language?.id ?? 1);
computed(() => userSettings.value?.dateFormat?.id ?? appSettings.value?.dateFormat?.id ?? 1);
computed(() => userSettings.value?.decimalSeperator?.id ?? appSettings.value?.decimalSeperator?.id ?? 1);
...
const isLoaded$1 = ref(false);
const loadedCallbacks = [];
const isDomainUser = ref(false);
const isLoggedIn = computed(() => !!token.value);
const currentUser = computed(() => username.value);
const userDefaultPage = computed(() => defaultPage.value);
const isUserDomain = computed(() => isDomainUser.value);
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return remoteComponent.value ? (openBlock(), createBlock(resolveDynamicComponent(remoteComponent.value), { key: 0 })) : (openBlock(), createElementBlock("p", _hoisted_1$1, "Loading remote module..."));
    };
  }
});
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(true), createElementBlock(Fragment, null, renderList(popups, (p, i) => {
        return openBlock(), createBlock(unref(DxPopup), {
          ...
          default: withCtx(() => [
            createElementVNode("div", _hoisted_1, [
              createElementVNode("div", _hoisted_2, toDisplayString(p.message), 1)
            ])
          ]),
          _: 2
...
//# sourceMappingURL=index.es-31fc6098.js.map
